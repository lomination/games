...
import lomination.games.core.shared.{Move, Outcome, Score, Turn, choose}
import scala.annotation.tailrec

private inline def max2[T <: Ordered[T]](a:T,b:T): T = if a >= b then a else b
...
case class AlphaBeta[G <: ZeroSumGame[G, M] & Evaluatable, M <: Move](
  ...
  @tailrec
  private def loopMoves(game: G, moves: List[M], depth: Int, alpha: Score, beta: Score, bestSoFar: Score): Score =
    moves match
      case head :: tail =>
        ...
        if newAlpha >= beta then 
          newAlpha // Beta cut-off
        else 
          loopMoves(game, tail, depth, newAlpha, beta, newBest)
      case Nil => bestSoFar
